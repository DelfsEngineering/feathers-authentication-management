...
module.exports.resetPwdWithLongToken = function (options, resetToken, password, params) {
  return Promise.resolve().then(function () {
    ensureValuesAreStrings(resetToken, password);

    return resetPassword(options, { resetToken: resetToken }, { resetToken: resetToken }, password, params);
  });
};
...
function resetPassword(options, query, tokens, password, params) {
  ...
  if (tokens.resetToken) {
    ...
    userPromise = users.get(id, params).then(function (data) {
      return getUserData(data, checkProps);
    });
  } else if (tokens.resetShortToken) {
    params.query = query;
    userPromise = users.find(params).then(function (data) {
      return getUserData(data, checkProps);
    });
  } else {
  ...
  return Promise.all([userPromise, hashPassword(options.app, password)]).then(function (_ref) {
    ...
    return patchUser(user, {
      ...
      return notifier(options.notifier, 'resetPwd', user1, params);
    }).then(function (user1) {
